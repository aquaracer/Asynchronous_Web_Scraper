...
class HeadHunter(base_jobsite):
    ...
    async def get_links(self, queue=queue):
        """Корутина для получения ссылок.
        ...
        async with get_session(self.service, self.browser) as session:
            ...
            await queue.put(None)  # после завершения цикла добавляем обьект None в конец очереди, чтобы обозначить
    ...
    async def fetch_content(self, queue=queue):
        ...
        await engine.execute(
             CreateTable(HeadHunter_db))  # создаем таблицу для хранения данных о вакансии на текущую дату
        while True:
            ...
            async with get_session(self.service, self.browser) as web_session:
                ...
                async with engine.connect() as conn:
                    async with conn.begin() as trans: # записываем данные в базу
